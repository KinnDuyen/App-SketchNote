...
class EditorViewModel @Inject constructor(
    ...
    private fun parseSketchDataToBlocks(sketchData: String): List<ContentBlock> {
        return try {
            sketchData.split("|||").mapNotNull { part ->
                when {
                    part.startsWith("TEXT:") -> {
                        val content = part.removePrefix("TEXT:")
                        val sepIdx = content.indexOf(":::")
                        val text = if (sepIdx >= 0) content.substring(0, sepIdx) else content
                        val highlightsRaw = if (sepIdx >= 0) content.substring(sepIdx + 3) else ""
                        val highlights = if (highlightsRaw.isBlank()) emptyList()
                        else highlightsRaw.split(";;;").mapNotNull { h ->
                            val parts = h.split(",")
                            if (parts.size < 3) null
                                ...
                                HighlightSpan(
                                    ...
                                    color = androidx.compose.ui.graphics.Color(parts[2].toULong())
                                )
                            } catch (_: Exception) { null }
                        }
                        ContentBlock.TextBlock(text = text, highlights = highlights)
                    }
                    part.startsWith("SKETCH:") -> {
                        val content = part.removePrefix("SKETCH:")
                        val parts = content.split(":")
                        ContentBlock.SketchBlock(
                            imagePath = parts.getOrElse(0) { "" },
                            heightDp = parts.getOrElse(1) { "200" }.toIntOrNull() ?: 200
                        )
                    }
                    else -> null
                }
    ...
    fun addSketchBlockFromImage(imagePath: String) {
        val newBlock = ContentBlock.SketchBlock(imagePath = imagePath)
        _contentBlocks.value = _contentBlocks.value + newBlock
        autoSave()
    }
            ...
            when (block) {
                is ContentBlock.TextBlock -> {
                    val highlightsJson = block.highlights.joinToString(";;;") { h ->
                        "${h.start},${h.end},${h.color.value}"  // ← đổi hashCode() thành .value
                    }
                    "TEXT:${block.text.replace("|||", " ")}:::$highlightsJson"
                    // ↑ dùng ::: thay vì : để tránh conflict với text có dấu :
                }
